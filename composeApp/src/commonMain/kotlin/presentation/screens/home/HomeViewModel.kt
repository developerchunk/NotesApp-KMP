...
class HomeViewModel(
    ...
    init {
        ...
        screenModelScope.launch(Dispatchers.Main) {
            delay(500)
            mongoDB.readActiveTasks().collect {
        ...
        screenModelScope.launch(Dispatchers.Main) {
            delay(500)
            mongoDB.readCompletedTasks().collect {
    ...
    fun setAction(action: TaskAction) {
        ...
        when(action) {
            is TaskAction.SetCompleted -> {
                setCompleted(task = action.task, completed = action.completed)
            }

            is TaskAction.SetFavorite -> {
                setFavorite(task = action.task, isFavorite = action.isFavorite)
            }
    ...
    private fun setCompleted(task: ToDoTask, completed: Boolean) {
        screenModelScope.launch(Dispatchers.IO) {
            mongoDB.setCompleted(task = task, taskCompleted = completed)
        }
    }

    private fun setFavorite(task: ToDoTask, isFavorite: Boolean) {
        screenModelScope.launch(Dispatchers.IO) {
            mongoDB.setFavorite(task = task, isFavorite = isFavorite)
        }
    }
